...
/**
 * Xaraya modification - if available via autoload don't load this
 */
...
class phpthumb
{
    ...
    public $config_file_create_mask                     = 0o755;
    public $config_dir_create_mask                      = 0o755;
        ...
        if ($commandline) {
            ...
            if ($IMtempfilename = $this->phpThumb_tempnam()) {
                ...
                foreach ($this->fltr as $filterkey => $filtercommand) {
                    [$command, $parameter] = array_pad(explode('|', $filtercommand, 2), 2, '');
                    switch ($command) {
                        case 'brit':
                        ...
                        case 'clr':
                            if ($this->ImageMagickSwitchAvailable(['fill', 'colorize'])) {
                                [$amount, $color] = array_pad(explode('|', $parameter), 2, '');
                        ...
                        case 'rcd':
                            if ($this->ImageMagickSwitchAvailable(['colors', 'dither'])) {
                                [$colors, $dither] = array_pad(explode('|', $parameter), 2, '');
                        ...
                        case 'lvl':
                            [$band, $method, $threshold] = array_pad(explode('|', $parameter), 3, '');
                        ...
                        case 'usm':
                            if ($this->ImageMagickSwitchAvailable('unsharp')) {
                                [$amount, $radius, $threshold] = array_pad(explode('|', $parameter), 3, '');
                        ...
                        case 'bord':
                            if ($this->ImageMagickSwitchAvailable(['border', 'bordercolor', 'thumbnail', 'crop'])) {
                                if (!$this->zc) {
                                    [$width, $rX, $rY, $color] = array_pad(explode('|', $parameter), 4, '');
                                    $width = (int) $width;
                                    $rX    = (int) $rX;
                                    $rY    = (int) $rY;
                                    if ($width && !$rX && !$rY) {
            ...
            foreach ($this->fltr as $filtercommand) {
                ...
                switch ($command) {
                    ...
                    case 'clr': // Colorize
                        ...
                        [$amount, $color] = array_pad(explode('|', $parameter, 2), 2, '');
                    ...
                    case 'rcd': // ReduceColorDepth
                        ...
                        [$colors, $dither] = array_pad(explode('|', $parameter, 2), 2, '');
                        $colors = ($colors ? (int) $colors : 256);
                        $dither  = ((strlen($dither) > 0) ? (bool) $dither : true);
                    ...
                    case 'bvl': // Bevel
                        [$width, $color1, $color2] = array_pad(explode('|', $parameter, 3), 3, '');
                        $phpthumbFilters->Bevel($this->gdimg_output, $width, $color1, $color2);
                        break;

                    case 'lvl': // autoLevels
                        [$band, $method, $threshold] = array_pad(explode('|', $parameter, 3), 3, '');
                    ...
                    case 'hist': // Histogram overlay
                        ...
                        [$bands, $colors, $width, $height, $alignment, $opacity, $margin_x, $margin_y] = array_pad(explode('|', $parameter, 8), 8, '');
                        $bands     = ($bands ? $bands : '*');
                    ...
                    case 'fram': // Frame
                        [$frame_width, $edge_width, $color_frame, $color1, $color2] = array_pad(explode('|', $parameter, 5), 5, '');
                    ...
                    case 'drop': // DropShadow
                        ...
                        $this->is_alpha = true;
                        [$distance, $width, $color, $angle, $fade] = array_pad(explode('|', $parameter, 5), 5, '');
                    ...
                    case 'mask': // Mask cropping
                        ...
                        [$mask_filename, $invert] = array_pad(explode('|', $parameter, 2), 2, '');
                    ...
                    case 'ric': // RoundedImageCorners
                        ...
                        [$radius_x, $radius_y] = array_pad(explode('|', $parameter, 2), 2, '');
                    ...
                    case 'crop': // Crop
                        [$left, $right, $top, $bottom] = array_pad(explode('|', $parameter, 4), 4, '');
                        $phpthumbFilters->Crop($this->gdimg_output, $left, $right, $top, $bottom);
                        break;

                    case 'bord': // Border
                        [$border_width, $radius_x, $radius_y, $hexcolor_border] = array_pad(explode('|', $parameter, 4), 4, '');
                        $this->is_alpha = true;
                        $phpthumbFilters->ImageBorder($this->gdimg_output, $border_width, $radius_x, $radius_y, $hexcolor_border);
                        break;

                    case 'over': // Overlay
                        [$filename, $underlay, $margin, $opacity] = array_pad(explode('|', $parameter, 4), 4, '');
                    ...
                    case 'wmi': // WaterMarkImage
                        [$filename, $alignment, $opacity, $margin['x'], $margin['y'], $rotate_angle] = array_pad(explode('|', $parameter, 6), 6, '');
                    ...
                    case 'wmt': // WaterMarkText
                        [$text, $size, $alignment, $hex_color, $ttffont, $opacity, $margin, $angle, $bg_color, $bg_opacity, $fillextend, $lineheight] = array_pad(explode('|', $parameter, 12), 12, '');
                        $text       = ($text ? $text : '');
                        $size       = ($size ? $size : 3);
                        $alignment  = ($alignment ? $alignment : 'BR');
                    ...
                    case 'usm': // UnSharpMask sharpening
                        [$amount, $radius, $threshold] = array_pad(explode('|', $parameter, 3), 3, '');
                        $amount    = ($amount ? $amount : 80);
                    ...
                    case 'size': // Resize
                        [$newwidth, $newheight, $stretch] = array_pad(explode('|', $parameter), 3, '');
        ...
        if (!is_null($this->ica)) {
            ...
            if (function_exists('imagecropauto')) { // (PHP 5 >= 5.5.0, PHP 7)
                ...
                if (preg_match('#^(([0-4])|(5)\\|(0?\\.?[0-9]+)\\|([0-9A-F]{6}))$#i', $this->ica, $matches)) {
                    [$dummy, $dummy, $ica_mode1, $ica_mode2, $ica_threshold, $ica_color] = $matches;
                    if ($ica_mode2) {
